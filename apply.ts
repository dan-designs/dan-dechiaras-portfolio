...
const tractionRegex = /<div className="bg-\[#1e1e1e\] border border-accent\/20 border-dashed p-8 rounded-lg text-center my-[0-9]+">\s*<div className="font-mono text-xs tracking-\[0\.15em\] uppercase text-accent\/80 mb-2">Image Placeholder - Traction<\/div>\s*<div className="text-sm text-\[#888\]">Analytics or usage dashboard.*<\/div>\s*<\/div>/g;
...
content = content.replace(
  /V1 shipped to the Apple App Store\. Development did not pause.*?fulfilled\./,
  "Version 1 shipped to the Apple App Store. Development did not pause. The second version of the native app has already rolled out, expanding the feature set. Meanwhile, we are now on version 4 of the promoter PWA. Once the foundation was established and I could begin testing, I was able to make rapid, progressive improvements."
);
...
// Replace "Request Case Study" button with "Visit Website" 
content = content.replace(/>\s*Request Case Study\s*</g, '>Visit Website<');

// 5b: attendee and promoter experience replaces "High-fidelity app screenshots..."
const screenshotRegex = /<div className="bg-\[#1e1e1e\] border border-accent\/20 border-dashed p-8 rounded-lg text-center my-[0-9]+">\s*<div className="font-mono text-xs tracking-\[0\.15em\] uppercase text-accent\/80 mb-2">Image Placement<\/div>\s*<div className="text-sm text-\[#888\]">High-fidelity app screenshots: the event feed, an event detail page, and the promoter dashboard.*<\/div>\s*<\/div>/g;
...
const designRealRegex = /<div className="bg-\[#1e1e1e\] border border-accent\/20 border-dashed p-8 rounded-lg text-center my-[0-9]+">\s*<div className="font-mono text-xs tracking-\[0\.15em\] uppercase text-accent\/80 mb-2">Image Placement<\/div>\s*<div className="text-sm text-\[#888\]">Side-by-side: A complex Figma screen on the left.*<\/div>\s*<\/div>/g;
...
const splitScreenRegex = /<div className="bg-\[#1e1e1e\] border border-accent\/20 border-dashed p-8 rounded-lg text-center my-[0-9]+">\s*<div className="font-mono text-xs tracking-\[0\.15em\] uppercase text-accent\/80 mb-2">Image Placement<\/div>\s*<div className="text-sm text-\[#888\]">Split-screen: Left - a vintage rave flyer aesthetic \/ Right - the Infolines app feed.*<\/div>\s*<\/div>/g;
...
const archRegex = /<div className="bg-\[#1e1e1e\] border border-accent\/20 border-dashed p-8 rounded-lg text-center my-[0-9]+">\s*<div className="font-mono text-xs tracking-\[0\.15em\] uppercase text-accent\/80 mb-2">Image Placement<\/div>\s*<div className="text-sm text-\[#888\]">Architectural diagram: Vite PWA \+ Expo App both feeding into Supabase.*<\/div>\s*<\/div>/g;
...
const promptRegex = /<div className="bg-\[#1e1e1e\] border border-accent\/20 border-dashed p-8 rounded-lg text-center my-[0-9]+">\s*<div className="font-mono text-xs tracking-\[0\.15em\] uppercase text-accent\/80 mb-2">Image Placement<\/div>\s*<div className="text-sm text-\[#888\]">Screenshot or diagram of the actual prompt workflow.*<\/div>\s*<\/div>/g;
...
const timelineRegex = /<div className="bg-\[#1e1e1e\] border border-accent\/20 border-dashed p-8 rounded-lg text-center my-[0-9]+">\s*<div className="font-mono text-xs tracking-\[0\.15em\] uppercase text-accent\/80 mb-2">Image Placement<\/div>\s*<div className="text-sm text-\[#888\]">Horizontal visual timeline with the same milestones rendered graphically.*<\/div>\s*<\/div>/g;
content = content.replace(timelineRegex, `<img src="https://res.cloudinary.com/datad8tms/image/upload/v1777847230/Timeline_aupcwn.png" alt="Visual 4-week sprint timeline" className="w-full rounded-lg my-10 border border-white/10" />`);
