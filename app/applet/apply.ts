...
const timelineRegex = /<div className="bg-\[#1e1e1e\] border border-accent\/20 border-dashed p-8 rounded-lg text-center my-10">\s*<div className="font-mono text-xs tracking-\[0\.15em\] uppercase text-accent\/80 mb-2">Image Placement<\/div>\s*<div className="text-sm text-\[#888\]">Horizontal visual timeline with the same milestones rendered graphically.*<\/div>\s*<\/div>/;
...
// Fallback if the timeline one had a my-6 class instead:
const timelineRegex2 = /<div className="bg-\[#1e1e1e\] border border-accent\/20 border-dashed p-8 rounded-lg text-center my-6">\s*<div className="font-mono text-xs tracking-\[0\.15em\] uppercase text-accent\/80 mb-2">Image Placement<\/div>\s*<div className="text-sm text-\[#888\]">Horizontal visual timeline with the same milestones rendered graphically.*<\/div>\s*<\/div>/;
content = content.replace(timelineRegex2, `<img src="https://res.cloudinary.com/datad8tms/image/upload/v1777847230/Timeline_aupcwn.png" alt="Visual 4-week sprint timeline" className="w-full rounded-lg my-6 border border-white/10" />`);

// Ensure 5 projects are shown on the home page instead of 4
// Let's replace "PROJECTS.slice(0, 4).map" or "PROJECTS.slice(0, 5).map" with "PROJECTS.map" which shows all of them if the array has 5, or limit to 5.
content = content.replace(/{PROJECTS\.slice\(\d+, \d+\)\.map\(/g, '{PROJECTS.map(');
